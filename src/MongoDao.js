...
(function() {
  var DAO, DBHandler, LOG, MongoDAO, mongoCondig;
  ...
  MongoDAO = (function() {
    class MongoDAO extends DAO {
      ...
      init() {
        ...
        this.projects = {};
        this.projects["conf"] = new DBHandler(perfix + "projects" + postfix, "conf", mongoCondig.DB_OPTS);
        this.projects["deploy"] = new DBHandler(perfix + "projects" + postfix, "deploy", mongoCondig.DB_OPTS);
        this.sys_user = {};
        this.sys_user["conf"] = new DBHandler(perfix + "sys_user" + postfix, "users", mongoCondig.DB_OPTS);
        this.sys_user["deploy"] = new DBHandler(perfix + "sys_user" + postfix, "session", mongoCondig.DB_OPTS);
        filter = {};
        if (argv.project) {
          filter = {
            name: argv.project
        ...
        this.projects.deploy.selectList(filter, (err, docs) => {
          ...
          for (i = 0, len = docs.length; i < len; i++) {
            ...
            if (proj_conf.task && proj_conf.task !== "") {
              ...
              this[proj_cache_db] = {};
              this[proj_cache_db]["task"] = new DBHandler(proj_conf.task, "task", mongoCondig.DB_OPTS);
              this[proj_cache_db]["examine"] = new DBHandler(proj_conf.task, "examine", mongoCondig.DB_OPTS);
            ...
            for (j = 0, len1 = bindList.length; j < len1; j++) {
              col = bindList[j];
              this[proj_conf.name][col] = new DBHandler(proj_conf.task, col, mongoCondig.DB_OPTS);
            }
