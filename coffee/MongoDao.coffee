...
class MongoDAO extends DAO
  ...
  init: ->
    ...
    @projects = {}
    @projects["conf"] = new DBProxy (new DBHandler perfix + "projects" + postfix, "conf", mongoCondig.DB_OPTS)
    @projects["deploy"] = new DBProxy (new DBHandler perfix + "projects" + postfix, "deploy", mongoCondig.DB_OPTS)
    @sys_user = {}
    @sys_user["conf"] = new DBProxy (new DBHandler perfix + "sys_user" + postfix, "users", mongoCondig.DB_OPTS)
    @sys_user["deploy"] = new DBProxy (new DBHandler perfix + "sys_user" + postfix, "session", mongoCondig.DB_OPTS)
    filter = {}
    ...
    @projects.deploy.selectList filter, (err, docs) =>
      ...
      for proj_conf in docs
        ...
        if proj_conf.task and proj_conf.task isnt ""
          ...
          @[proj_cache_db] = {}
          @[proj_cache_db]["task"] = new DBProxy (new DBHandler proj_conf.task, "task", mongoCondig.DB_OPTS)
          @[proj_cache_db]["examine"] = new DBProxy (new DBHandler proj_conf.task, "examine", mongoCondig.DB_OPTS)
        ...
        @[proj_conf.name] = {}
        @[proj_conf.name][col] = new DBProxy (new DBHandler proj_conf.task, col, mongoCondig.DB_OPTS) for col in bindList
